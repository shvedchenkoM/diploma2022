...
void LittleSolver::handleMatrix(const Matrix<double> &m, const arclist &path, double bottomLimit) {
    if((double )(clock() - tStart)/CLOCKS_PER_SEC > 8)
    {
...
void LittleSolver::candidateSolution(const arclist &arcs) {
    ...
    _arcs = arcs;
    ans.push_back("");
    ans[ans.size() - 1] += std::to_string(_record) + ':';
    std::map<int, int> mT;
    for (auto elem: _arcs) {
        mT[elem.first] = elem.second;
    }
    int start = 0;
    ans[ans.size() - 1] += std::to_string(start) + ' ';
    start = mT[start];
    while (start != 0) {
        ans[ans.size() - 1] += std::to_string(start) + ' ';
        start = mT[start];
